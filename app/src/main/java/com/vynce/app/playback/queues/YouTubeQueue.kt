package com.vynce.app.playback.queues
...
class YouTubeQueue(
    ...
    private val usedVideoIds: MutableSet<String> = mutableSetOf(),
) : Queue {
    ...
    override suspend fun nextPage(): List<MediaMetadata> {
        ...
        continuation = nextResult?.continuation
        val newSongs = nextResult?.items?.map { it.toMediaMetadata() } ?: emptyList()
        val freshSongs = newSongs.filter { it.id !in usedVideoIds }
        usedVideoIds.addAll(freshSongs.map { it.id })
        return freshSongs
    }
...
//    }
    ...
    companion object {
        private val usedVideoIds = mutableSetOf<String>()

        fun radio(song: MediaMetadata) = YouTubeQueue(WatchEndpoint(song.id), song, usedVideoIds = usedVideoIds)
