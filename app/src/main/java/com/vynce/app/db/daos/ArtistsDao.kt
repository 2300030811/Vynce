...
interface ArtistsDao {
    ...
    @Query("""
        ...
        LIMIT :limit
    """)
    fun mostPlayedArtists(limit: Int = 5): Flow<List<com.vynce.app.db.ArtistWithPlayCount>>
    ...
    @Query("""
        ...
        LEFT JOIN playCount ON sam.songId = playCount.song
        LEFT JOIN song ON sam.songId = song.id
        WHERE song.inLibrary IS NOT NULL AND playCount.year = :year AND playCount.month = :month
        GROUP BY artist.id
        ORDER BY playCount DESC
        LIMIT :limit
    """)
    fun mostPlayedArtists(year: Int, month: Int, limit: Int = 5): Flow<List<com.vynce.app.db.ArtistWithPlayCount>>
