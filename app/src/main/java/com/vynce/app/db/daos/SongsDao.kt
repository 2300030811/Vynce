...
interface SongsDao {
    ...
    @Transaction
    @Query("""
        SELECT song.*, IFNULL(SUM(playCount.count), 0) as playCount
        FROM song
        LEFT JOIN playCount ON playCount.song = song.id
        WHERE song.inLibrary IS NOT NULL
        GROUP BY song.id
        ORDER BY playCount DESC
        LIMIT :limit
    """)
    fun mostPlayedSongs(limit: Int = 10): Flow<List<com.vynce.app.db.SongWithPlayCount>>
    ...
    @Query("""
        ...
        WHERE event.timestamp > :fromTimeStamp AND song.inLibrary IS NOT NULL
        GROUP BY song.id
        ORDER BY SUM(event.playTime) DESC
        LIMIT :limit OFFSET :offset
    """)
    fun mostPlayedSongs(fromTimeStamp: Long, limit: Int = 10, offset: Int = 0): Flow<List<Song>>
    @Query("SELECT IFNULL(SUM(playCount.count), 0) FROM playCount INNER JOIN song ON playCount.song = song.id WHERE song.inLibrary IS NOT NULL")
    fun totalPlayCount(): Flow<Int>

    @Query("SELECT IFNULL(SUM(playCount.count * song.duration / 60000), 0) FROM playCount INNER JOIN song ON playCount.song = song.id WHERE song.inLibrary IS NOT NULL")
    fun totalListeningMinutes(): Flow<Int>
